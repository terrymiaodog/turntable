...
uint8_t g_rx_buffer[RXBUFFERSIZE];                    /*HAL库使用的串口1接收缓冲 */

uint8_t g_usart_title=0;                              /*串口1是否接收到正确帧头，1为接收到，0为没有 */

uint8_t g_usart6_rx_buf[USART6_REC_LEN];
uint16_t g_usart6_rx_sta = 0;
uint8_t g_rx6_buffer[RXBUFFERSIZE6];                  /*HAL库使用的串口6接收缓冲 */

UART_HandleTypeDef g_uart1_handle;                    /*UART句柄*/
UART_HandleTypeDef g_uart6_handle;                    /*UART6句柄*/
/**
 * @brief       串口1初始化函数
...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
    ...
    {
        USART6_UX_CLK_ENABLE();                                  /* USART6 时钟使能 */
...
#if USART6_EN_RX
        HAL_NVIC_EnableIRQ(USART6_UX_IRQn);                      /* 使能USART6中断通道 */
        HAL_NVIC_SetPriority(USART6_UX_IRQn, 3, 3);              /* 抢占优先级3，子优先级3 */
#endif
    }
}
...
{
    if(huart->Instance == USART6_UX)                             /* 如果是串口6 ，使用定时器进行帧判断*/
    {
    ...
    if(huart->Instance == USART_UX)                              /* 如果是串口1 ，使用固定字节数+起始字节进行帧判断*/
    {
        if((g_usart_rx_sta & 0x8000) == 0){                      /* 接收未完成 */
            if(g_usart_rx_sta==0 && g_rx_buffer[0]==0xDF){
                ...
                if( g_usart_rx_sta == 0x0010){                       /* 接收到多少字节，转入处理 */
